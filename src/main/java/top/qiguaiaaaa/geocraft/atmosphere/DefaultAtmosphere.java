...
public class DefaultAtmosphere extends BaseAtmosphere implements INBTSerializable<NBTTagCompound> {
    ...
    public void updateTick(Chunk chunk){
        tickTimes++;
        if(debug) GEOInfo.getLogger().info("{} {} Atmosphere updated {}",chunk.x,chunk.z,tickTimes);
        ...
        for(Layer layer:layers){
            if(debug) GEOInfo.getLogger().info("{} {} Atmosphere is updating layer {} ,i = {}",chunk.x,chunk.z,layer.getTagName(),layers.indexOf(layer));
            layer.tick(chunk,neighbors);
        }
        if(debug) GEOInfo.getLogger().info("{} {} Atmosphere updated successfully, now status:\n {}",chunk.x,chunk.z,this.toString());
        //更新Listener
