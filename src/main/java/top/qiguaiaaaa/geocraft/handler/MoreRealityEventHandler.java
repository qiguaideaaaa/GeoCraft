...
public final class MoreRealityEventHandler {
    ...
    public void onBucketEvent(FillBucketEvent event){
        ...
        if(item == Items.WATER_BUCKET){
            if(GeoFluidProperty.isFluidToUseVanillaBucketMode(FluidRegistry.WATER)) return;
            ...
            if(GeoFluidProperty.isFluidToUseVanillaBucketMode(FluidRegistry.LAVA)) return;
    ...
    public static void onPostInit(FMLPostInitializationEvent event){
        ...
        for(ConfigurableFluid fluid:fluidsWhoseBucketsBehavesAsVanillaBuckets.getValue()){
            if(fluid == null) continue;
            GeoFluidProperty.setFluidToUseVanillaBucketMode(fluid.toString(),true);
        }
        if(!GeoFluidProperty.isFluidToBePhysical(FluidRegistry.WATER)){
            Blocks.WATER.setTickRandomly(false);
        }
        if(!GeoFluidProperty.isFluidToBePhysical(FluidRegistry.LAVA)){
