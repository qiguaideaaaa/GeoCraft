...
public class TemperatureTracker extends InformationLoggingTracker {
    ...
    public void notifyListener(Atmosphere atmosphere) {
        AtmosphereLayer layer = atmosphere.getUnderlying().getUpperLayer();
        if(layer == null) {
            FGInfo.getLogger().info("couldn't tract atmosphere temp: GroundAtmosphereLayer not found");
            return;
        }
        String msg = String.format("%d,%f,%f",atmosphere.getAtmosphereWorldInfo().getWorld().getTotalWorldTime(),layer.getTemperature().getCelsius(),
