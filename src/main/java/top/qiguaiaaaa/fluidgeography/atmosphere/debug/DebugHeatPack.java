package top.qiguaiaaaa.fluidgeography.atmosphere.debug;
...
public class DebugHeatPack extends HeatPack {
    ...
    public DebugHeatPack(HeatType type, double raw) {
        super(type, raw);
        FGInfo.getLogger().info("Heat Pack {} created, Q={}, Type = {}",id,raw,type.name());
    }

    @Override
    public double drawHeat(double amount) {
        FGInfo.getLogger().info("Heat Pack {} is drawn by {} FE, old = {} FE, new = {} FE",id,amount,heat,Math.max(heat-amount,0));
