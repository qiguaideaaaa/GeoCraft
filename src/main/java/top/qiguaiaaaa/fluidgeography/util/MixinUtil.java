...
public final class MixinUtil {
    ...
    public static List<String> getMoreRealityModeMixins(List<String> mixinList){
        if(SimulationConfig.enableSupportForIC2.getValue().value && Loader.isModLoaded("ic2"))
            mixinList.add("mixins/ic2/mixins.fluidgeography_reality.json");
        if(SimulationConfig.enableSupportForIE.getValue().value && Loader.isModLoaded("immersiveengineering"))
            mixinList.add("mixins/immersiveengineering/mixins.fluidgeography_reality.json");
        return mixinList;
    }
