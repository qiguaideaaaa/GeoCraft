# 测试：镍 API-NBT 路径-语法解析与读扫一致性

## 测试描述

本目录存放 NBTPath 路径语法的参数化测试数据，供 `清汩萌.镍.snbt` 包下的测试类使用。共两个数据文件。

### `解析用例.yaml`（122 条）

验证路径读取器（`NBTPathReader.readPathFromInput`）的正确性，供 `TestNBTPathParse` 使用。覆盖以下语法：

- 简单键，以及 `.` 分隔的嵌套键；
- 列表索引：含负索引、`+` 号、`-0` / `+0`、`int` 上下界与溢出；
- `[]` 全部节点；
- `[{...}]` 列表复合过滤、`{...}` 根复合过滤、键值过滤；
- 过滤器内的 SNBT 函数调用：决议成功与未注册报错两种。过滤复合经 `SNBTReader` 解析，函数在解析期决议求值；
- 索引与过滤器的毗连混排：含"过滤器复合只能在首位"的错位报错，方法节点之后毗连复合同样错位；
- 带引号的键：空格、点、中文、emoji、特殊字符、各类转义序列、纯空白名；
- 方法调用节点：根部直接调用、多级方法链、引号方法名、实参表截断、实参含函数调用、方法后毗连索引或列表过滤、无参调用有参方法；
- 超长路径（64 层）；
- 各类非法语法的报错：点号开头、连续多点、全空白输入等。

### `读扫一致性.yaml`（52 条）

把同一输入分别交给读取器（`NBTPathReader`）与扫描器（`NBTPathScanner`），对照两者的结论，供 `TestNBTPathScanConsistency` 使用。结论不一致或需要背景说明的用例，在 `注` 字段写明缘由。

不一致都源于扫描器只做浅层语法扫描、读取器做深层校验，具体三类：

- 扫描器缺的语法位置校验：复合过滤器位置检查（含方法或索引之后毗连复合）、空引号名的识别。数据的 `注` 字段把这些差异标为作者已确认的预期行为。
- 扫描器不做的决议类检查：方法注册、方法重载、过滤器内的 SNBT 函数决议。读者做这些检查而扫描器不做，于是产生同族的"读者报错 / 扫描通过"差异，每条差异都有用例记录现状。
- 扫描器不做的实参取值校验：非法正则、类型编号越界这类实参值本身的合法性检查，同样只有读取器报错。

### 宽容行为

路径解析的宽容行为都是预期行为，用例中有对应条目，例如 `a. b` 里点后的空白被跳过。尾随的 `.` 属截断输入，读者报错、扫描器发 EOF 信号。

## 样例格式

数据文件是 UTF-8 编码的 YAML，顶层是用例列表。

`解析用例.yaml` 每条用例的字段：

- `名`：用例名，作为 JUnit 参数化报告里的显示名；
- `输入`：待解析的路径原文；
- `期望`：可选，期望的 `NBTPath#toString()` 结果。注意 toString 的输出与解析语法不完全一致：节点之间一律以 `.` 连接（如 `a[0]` 的 toString 是 `a.[0]`），方法节点的 toString 是注册签名（如 `a.type("INT")` 的 toString 是 `a.type(STRING)`）。含过滤器的节点因过滤器字符串形态不稳定，不写 `期望`，只写 `长度`；
- `长度`：可选，期望的节点数（`NBTPath#length()`）；
- `异常`：可选布尔值，默认 `false`。取 `true` 时期望解析抛出 `CommandException`；
- `键`：可选，期望异常信息组件树里出现的翻译键（仅在能确定具体报错键时填写）；
- `剩余`：可选，解析在空白处停止后，输入流里剩余的原文。

`读扫一致性.yaml` 每条用例的字段：

- `名`：用例名；
- `输入`：路径原文；
- `读者`：读者的期望结论，取 `通过` 或 `报错`（`CommandException`）；
- `扫描`：扫描器的期望结论，取 `通过`、`报错` 或 `信号`（`NickelScanEOFSignal`，表示输入被视作截断）；
- `注`：可选，对该用例的补充说明（主要用于两者结论不一致的缘由，一致的用例也可用）。

## 样例 #1

### 输入 #1

```yaml
- 名: 列表索引
  输入: a[0]
  期望: a.[0]
  长度: 2
```

## 样例 #2

### 输入 #2

```yaml
- 名: 未定义方法
  输入: nosuchmeth(1)
  读者: 报错
  扫描: 通过
  注: 扫描器只扫方法实参语法，不检查方法是否注册；结论不一致，为预期行为
```

## 说明/提示

### 样例 1 解释：

`a[0]` 解析为两个节点：标签节点 `a` 与索引节点 `[0]`。`NBTPath#toString()` 把所有节点以 `.` 连接，因此结果是 `a.[0]` 而非原文 `a[0]`。

### 样例 2 解释：

`nosuchmeth` 未在 `NBTPathMethods` 注册。读取器在解析期决议方法失败而报错（`nickel.command.nbt.path.method.undefined`）；扫描器只做语法层面的实参扫描，不做方法决议，因此通过。该差异属现状行为，一致性测试按 `注` 里记录的期望分别断言两侧。

### 其他约定

- 含方法调用的用例，要求测试先经 `NBTPathMethods.loadFuncs(NBTPathMethods.class)` 注册内置方法。重复注册只会生效一次，注册时绕开 FML 的 `scanProviders`。
- `信号` 结论对应扫描器的补全语义：输入在语法完整之前断流时，扫描器抛 `NickelScanEOFSignal` 而非报错。
