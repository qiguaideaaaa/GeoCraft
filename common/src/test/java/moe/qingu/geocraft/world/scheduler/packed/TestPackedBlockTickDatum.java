...
public final class TestPackedBlockTickDatum extends TestBlockTickDatum {
    ...
    public void 测试打包反序列化(final @Nonnull 打包反序列化与序列化测试项 data) throws ClassNotFoundException, InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        test(new Object[]{data.in,data.ans});
    }

    @ParameterizedTest
    @MethodSource("为测试打包序列化准备数据")
    public void 测试打包序列化(final @Nonnull 打包反序列化与序列化测试项 data) throws ClassNotFoundException, InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        test(new Object[]{data.in,data.ans});
    }
    ...
    public static void 测试打包序列化_Inner(final @Nonnull String in,final @Nonnull String ans) throws CommandException {
        ...
        $计划刻数据.初始化();
        final PackedBlockTickDatum datum = new PackedBlockTickDatum();
        $计划刻数据.ticks.forEach(t -> datum.schedule($计划刻数据.time, t.x,t.y,t.z, Block.REGISTRY.getIDForObject(t.block()),t.时-$计划刻数据.time,t.priority()));
        datum.markDirty();
    ...
    public static Stream<打包反序列化与序列化测试项> 为测试打包反序列化准备数据(){
        ...
        ClassGraphUtils.findInputs("data/world/schedule/packed/（反）序列化/","snbt",consumer);
        return list.stream();
    }
    ...
    public static Stream<打包反序列化与序列化测试项> 为测试打包序列化准备数据(){
        ...
        ClassGraphUtils.findInputs("data/world/schedule/packed/序列化/","yaml",consumer);
        return list.stream();
    }
