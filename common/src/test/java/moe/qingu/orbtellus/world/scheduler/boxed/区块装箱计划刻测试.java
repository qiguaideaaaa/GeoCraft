...
import moe.qingu.orbtellus.world.scheduler.方块计划刻调度器测试;
import moe.qingu.orbtellus.world.scheduler.计划刻数据;
 ...
 * @author QGMoe
 */
public final class 区块装箱计划刻测试 extends 方块计划刻调度器测试 {

    @ParameterizedTest
    @MethodSource("为测试装箱反序列化准备数据")
    public void 测试装箱反序列化(final @Nonnull 装箱反序列化与序列化测试项 $项) throws ClassNotFoundException, InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        test(new Object[]{$项.in,$项.ans,$项.x,$项.z});
    }

    @ParameterizedTest
    @MethodSource("为测试装箱序列化准备数据")
    public void 测试装箱序列化(final @Nonnull 装箱反序列化与序列化测试项 $项) throws ClassNotFoundException, InvocationTargetException, NoSuchMethodException, IllegalAccessException {
        test(new Object[]{$项.in,$项.ans,$项.x,$项.z});
    }
    ...
    public static void 测试装箱序列化_Inner(final @Nonnull String in,final @Nonnull String ans,final int x,final int z) throws CommandException {
        ...
        $计划刻数据.初始化();
        final World world = 模拟沙盒世界.构建(模拟世界配置.create(b -> b.withTotalTime($计划刻数据.time)),false);
        final EmptyChunk chunk = new EmptyChunk(world,x,z);
        final BoxedBlockTickDatum datum = new BoxedBlockTickDatum(chunk);
        final int baseX = x << 4;
        final int baseZ = z << 4;
        $计划刻数据.ticks.forEach(t ->{
            Assertions.assertTrue(MathUtil.inRangeClose(t.$于[0],baseX,baseX+15),t.$于[0]+" doesn't fit chunk "+baseX +" to "+(baseX+15));
            Assertions.assertTrue(MathUtil.inRangeClose(t.$于[2],baseZ,baseZ+15),t.$于[2]+" doesn't fit chunk "+ baseZ +" to "+(baseZ+15));
        });
    ...
    public static Stream<装箱反序列化与序列化测试项> 为测试装箱反序列化准备数据(){
        ...
        ClassGraphUtils.寻找样例输入("data/world/schedule/boxed/（反）序列化/","snbt", consumer);
        return list.stream();
    }
    ...
    public static Stream<装箱反序列化与序列化测试项> 为测试装箱序列化准备数据(){
        ...
        ClassGraphUtils.寻找样例输入("data/world/schedule/boxed/序列化/","yaml", consumer);
        return list.stream();
    }
