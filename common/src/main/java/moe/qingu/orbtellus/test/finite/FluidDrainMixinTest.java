...
package moe.qingu.orbtellus.test.finite;

import moe.qingu.orbtellus.mixin.finite.ForgeFluidUtilMixin;
...
public final class FluidDrainMixinTest extends GeoTestItem {
    ...
    public FluidDrainMixinTest(){
        this.id = new ResourceLocation(OrbTellusCraft.MODID,"finite_fluid_drain_mixin_test");
    }
    ...
    private static EnumActionResult validate(final @Nonnull World world,final @Nonnull BlockPos pos,final @Nullable ICommandSender sender){
        ...
        final Fluid fluid = moe.qingu.orbtellus.api.util.FluidUtil.getFluid(state);
