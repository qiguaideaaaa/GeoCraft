...
import moe.qingu.orbtellus.fluid.FluidCarbonDioxide;
import moe.qingu.orbtellus.fluid.FluidSnow;

public final class FluidHandler {
    public static void initRegisteredFluids(){
        if(OTCFluids.CARBON_DIOXIDE == null){
            OTCFluids.CARBON_DIOXIDE = getValidFluid(FluidCarbonDioxide.fluidName,new FluidCarbonDioxide());
        }
        if(OTCFluids.SNOW == null){
            OTCFluids.SNOW = getValidFluid(FluidSnow.fluidName,new FluidSnow());
        }
    }
