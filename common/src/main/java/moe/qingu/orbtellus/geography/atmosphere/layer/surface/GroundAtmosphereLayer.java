...
import static moe.qingu.orbtellus.api.atmosphere.Atmosphere.TEMPERATURE_NOISE;
...
public class GroundAtmosphereLayer extends SurfaceAtmosphereLayer {
    ...
    protected void 热量对流(double 垂直风速){
        ...
        if(((SurfaceAtmosphere)atmosphere).isDebug())
            OrbTellusCraft.getLogger().info("{} flow heat {} FE to UP ({} K changed),wind = {}",
    ...
    protected void initUnderlyingTemp(){
        if(lowerLayer != null){ // 设置下垫面温度
            ...
            TemperatureState deepTemperature = lowerLayer.getTemperature(OTCProperties.DEEP_TEMPERATURE);
