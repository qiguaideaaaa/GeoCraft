...
package moe.qingu.orbtellus.geography.atmosphere.tracker;
...
public class FluidTracker extends InformationLoggingTracker {
    ...
    public void postAtmosphereTick(@Nonnull Atmosphere atmosphere) {
        ...
        if(layer == null){
            OrbTellusCraft.getLogger().warn("Couldn't track {} at {} , because there doesn't exist an atmosphere layer.",propertyToTrack.getRegistryName(),pos);
            return;
        }
        GeographyState s = layer.getState(propertyToTrack);
        if(s == null){
            OrbTellusCraft.getLogger().warn("Couldn't track {} at {} , because the layer there doesn't have this property.",propertyToTrack.getRegistryName(),pos);
            return;
        }
        if(!(s instanceof FluidState)){
            OrbTellusCraft.getLogger().warn("Couldn't track {} at {} , because this isn't a valid fluid state.",propertyToTrack.getRegistryName(),pos);
            return;
        }
        ...
        logger.println(msg);
        OrbTellusCraft.getLogger().info("track {} ({} mB)",propertyToTrack.getRegistryName(),msg);
        nowTime++;
