...
package moe.qingu.orbtellus.geography.atmosphere.layer.surface;
...
import moe.qingu.orbtellus.geography.atmosphere.debug.DebugHeatPack;
import moe.qingu.orbtellus.geography.atmosphere.layer.QiguaiAtmosphereLayer;
...
public abstract class SurfaceAtmosphereLayer extends QiguaiAtmosphereLayer {
    ...
    protected void 热量平流(Atmosphere to,EnumFacing dir) {
        ...
        if(((SurfaceAtmosphere)atmosphere).isDebug())
            OrbTellusCraft.getLogger().info("{} flow heat {} FE to {} ({} K changed),wind = {}. temperature diff = {} K .to temp {} , to layer {} ,to pressure {} Pa . me pressure {}, me density {}",
    ...
    protected void 水汽凝结(){
        ...
        if(((SurfaceAtmosphere)atmosphere).isDebug())
            OrbTellusCraft.getLogger().info("{} has water pressure {} Pa > {} Pa ,should transfer {} mB = " +
                            " {} - {} * {} * {} * {} / ({} * {})"+
    ...
    public void tick(@Nullable Chunk chunk, @Nonnull Map<EnumFacing, Triple<Atmosphere,Chunk,EnumFacing>> neighbors, int x, int z) {
        ...
        if(((SurfaceAtmosphere)atmosphere).isDebug())
            OrbTellusCraft.getLogger().info("{} send radiation by {} FE (-{} K)",getTagName(),总辐射量,总辐射量/heatCapacity);
        ...
        for(Triple<Atmosphere,Chunk,EnumFacing> neighbor:neighbors.values()){
            ...
            if(((SurfaceAtmosphere)atmosphere).isDebug()) OrbTellusCraft.getLogger().info("{} calculated wind {} as {} ",getTagName(),direction.name(),newWindSpeed);
    ...
    public void sendHeat(@Nonnull HeatPack pack, @Nullable EnumFacing direction) {
        ...
        switch (pack.getType()){
            ...
            case LONG_WAVE:
                ...
                透过率 = 1.0 - 吸收率;
                if(((SurfaceAtmosphere)atmosphere).isDebug())
                    OrbTellusCraft.getLogger().info("{} received LONG WAVE pack, will pass {} and absorb {} ,dir is {}",
    ...
    public void sendHeat(@Nonnull HeatPack pack, @Nullable Vec3d direction) {
        ...
        switch (pack.getType()){
            case SHORT_WAVE:
                ...
                吸收率 = 参数[1];
                if(((SurfaceAtmosphere)atmosphere).isDebug())
                    OrbTellusCraft.getLogger().info("{} received SHORT WAVE pack, will pass {} and absorb {} ,degree is {}",
